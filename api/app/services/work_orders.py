...
def list_work_orders(db: Session, *, tenant_id: int) -> list[WorkOrder]:
    stmt = (
        select(WorkOrder)
...
def get_work_order_or_404(db: Session, *, tenant_id: int, work_order_id: int) -> WorkOrder:
    wo = db.execute(
...
def start_work_order(db: Session, *, current_user: User, work_order_id: int) -> WorkOrder:
    ...
    wo = get_work_order_or_404(db, tenant_id=current_user.tenant_id, work_order_id=work_order_id)
    ...
    if active and active.work_order_id != wo.id:
        other = get_work_order_or_404(db, tenant_id=current_user.tenant_id, work_order_id=active.work_order_id)
